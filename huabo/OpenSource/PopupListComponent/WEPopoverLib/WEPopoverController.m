...
- (void)presentPopoverFromRect:(CGRect)rect 
    ...
    CGRect rect1 = CGRectMake(rect.origin.x, rect.origin.y-44, rect.size.width, rect.size.height);
    //-----------------------------------------------------------------------
	...
	WEPopoverContainerView *containerView = [[WEPopoverContainerView alloc] initWithSize:self.popoverContentSize anchorRect:rect1 displayArea:displayArea permittedArrowDirections:arrowDirections properties:props];
...
- (CGRect)displayAreaForView:(UIView *)theView {
	...
	if ([theView conformsToProtocol:@protocol(WEPopoverParentView)] && [theView respondsToSelector:@selector(displayAreaForPopover)]) {
        ...
        //NSLog(@"%f %f",displayArea.size.width,displayArea.size.height);
	}
	return displayArea;
}
